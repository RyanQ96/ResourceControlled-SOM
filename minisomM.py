...
class MiniSom(object):
    ...
    def train(self, data, num_iteration, random_order=False, verbose=False):
        ...
        for t, iteration in enumerate(iterations): 
            if t % len(data) == 0:
                batch_idx += 1
                print(f"working on {batch_idx}th batch right now, updating resource, status: {self._resource_map.max(), self._resource_map.min()}")
                self._resource_initialize()
                print(f"finished updating resource, status: {self._resource_map.max(), self._resource_map.min()}")
